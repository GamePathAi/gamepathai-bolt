/*
  # Security Events and User Activity Logging

  1. New Tables
    - `security_events` - Stores security-related events with severity levels
    - `user_activity_logs` - Tracks user activities for behavior analysis
  
  2. Functions
    - `log_security_event` - Helper function to log security events
    - `log_user_activity` - Helper function to log user activities
    - `detect_suspicious_activity` - Detects unusual patterns in user activity
    - `monitor_failed_logins` - Monitors for potential brute force attempts
  
  3. Policies
    - RLS policies for both tables to ensure proper access control
...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE FUNCTION detect_suspicious_activity() RETURNS trigger AS $$
    DECLARE
...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE FUNCTION monitor_failed_logins() RETURNS trigger AS $$
    DECLARE
