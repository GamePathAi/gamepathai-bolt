  ...
  try {
    ...
    if (window.electronAPI.system.platform === "win32") {
      try {
        ...
        console.error("Error reading Steam game:", error);
      }
    }
    
    // Caminhos padrão para diferentes plataform (!steamPath) {
      const defaultPaths = {
        win32: [
    ...
    // Encontrar  de jogos do Steam
    const libraries = [steamPath];
    
    // Ler o arquivo libraryfolders.vdf para encontrar mais bibliotec {
          ...
          for (const manifestFile of manifestFiles) {
            try {
              ...
              if (appIdMatch && nameMatch && installDirMatch) {
                ...
                const sizeInBytes = sizeOnDiskMatch ? parseInt(sizeOnDiskMatch[1]) ;
