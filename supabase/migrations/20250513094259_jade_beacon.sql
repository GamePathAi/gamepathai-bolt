/*
  ...
  2. Security
    ...
    - Add trigger for installation status updates
...
-- Add installation status constraint, checking if it already exists first
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'valid_installation_status' AND contable = 'download_events'::regclass) THEN
...
CREATE POLICY IF NOT EXISTS "Allow download event creation"
  ON download_events
  FOR INSERT
...
CREATE POLICY IF NOT EXISTS "Allow users to view their own download events"
  ON download_events
  FOR SELECT
  TO authenticated
