/*
  # Security and User Profile Schema

  1. New Tables
    - `security_logs` - Tracks security events like logins, logouts, etc.
    - `user_profiles` - Stores user profile information and subscription details
    - `security_audit_logs` - Stores detailed security audit information
...
-- Create trigger to update updated_at on user_profiles (check if it exists first)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Create trigger to create user profile on new user (check if it exists first)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
...
-- Create trigger for monitoring failed logins (check if it exists first)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
...
-- Create trigger for trial status checking (check if it exists first)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
...
-- Create trigger for trial expiration notification (check if it exists first)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
