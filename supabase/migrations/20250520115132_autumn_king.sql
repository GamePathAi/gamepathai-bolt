/*
  ...
  1. New Tables
    ...
    - `user_activity_logs` - Tracks user behavior for analysis
  
  2. Functions
    - `log_security_event` - Logs security events with contextual information
    - `log_user_activity` - Records user activities with resource information
...
-- Create policies for security_events (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create policies for user_activity_logs (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create indexes for faster security event queries (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
